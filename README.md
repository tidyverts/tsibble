...
windows using purrr-like syntax:
  ...
  - `slide()`/`slide2()`/`pslide()`/`lslide()`: sliding window with
    overlapping observations.
  - `tile()`/`tile2()`/`ptile()`/`ltile()`: tiling window without
    overlapping observations.
  - `stretch()`/`stretch2()`/`pstretch()`/`lstretch()`: fixing an
    initial window and expanding to include more observations.
