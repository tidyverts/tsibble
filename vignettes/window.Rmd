...
The `pedestrian` dataset includes hourly pedestrian counts in the city of Melbourne, with `Sensor` as key and `Date_Time` as index. These windowed functions are index-based rolling for tackling general problems, rather than time indexed. Implicit missing values are thereby made explicit using `fill_na()`, and `.full = TRUE` warrants the equal time length of each sensor. This prepares the data inputs in the expected order.
...
pedestrian_mth %>% 
  group_by(Sensor) %>% 
  mutate(Monthly_MA = pslide(data, 
    quarter_mean, .size = 3, .align = "center"
  ))
x <- pslider(!!! pedestrian_mth %>% filter(Sensor == "Birrarung Marr"), .size = 3)
pedestrian_mth %>% 
  group_by(Sensor) %>% 
  mutate(Monthly_MA = pslide_dbl(!!! data, 
    quarter_mean, .size = 3, .align = "center"
  ))
pedestrian_mth %>% 
  group_by(Sensor) %>% 
  mutate(Monthly_MA = pslider(data, 
    .size = 3,.partial = TRUE
  ))
```
