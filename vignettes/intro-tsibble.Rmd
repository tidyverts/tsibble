...
It appears more advantageous to specify the structural variables using this syntax when coming to hierarchical and grouped time series forecast.

## Interval

It has been seen that the tsibble handles regularly-spaced temporal data well, from seconds to years. The option `regular`, by default, is set to `TRUE` in the `as_tsibble()`. Turn `regular` to `FALSE` to create a tsibble for the data collected at irregular time interval. Here shows the scheduled date time of the flights in New York City.
...
The keys are comprised of the `flight`, `origin`, and `dest` variables. With `regular = FALSE`, it turns to an irregularly-spaced tsibble, where `[!]` highlights the irregularity.
