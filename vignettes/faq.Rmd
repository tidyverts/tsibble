...
The interval depends on time representation. Class `yearmonth` is the accurate representation for monthly data. It is unclear to to tell if it's daily data with implicit missingness or it's monthly data. This applies to `POSIXct` for sub-daily data, `Date` for daily, `yearquarter` for quarterly, and etc.
...
Each station shares the same 15-minute interval, but the date-times don't align. Rounding them is a quick way to fix it, if binning time doesn't make much difference to the analysis. If it does, please organise them in different tables.
