...
While collapsing rows (like `summarise()`), `group_by()` and `index_by()` will update the key and index respectively. This `index_by()` + `summarise()` combo can help with regularising a tsibble of irregular time space too.
...
It can be noticed that the tsibble seamlessly works with *tidyverse* verbs. Use `?tsibble::reexports` for a full list of re-exported functions.
