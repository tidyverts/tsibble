...
test_that("stretch2() and its variants", {
  ...
  expect_equal(
    stretch2_dfr(df, df, ~ tibble::tibble(x = sum(.x, .y)), .size = 2, .flatten = TRUE)[1, 1, drop  = TRUE],
    sum(df[1, ]) * 2
  )
  expect_equal(
    stretch2_dfc(df, df, ~ tibble::tibble(x = sum(.x, .y)), .size = 2, .flatten = TRUE)[1, 1, drop  = TRUE],
    sum(df[1, ]) * 2
  )
})
