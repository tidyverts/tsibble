...
test_that("slide2()", {
  expect_equal(
    slide2_dfr(x, y, ~ data.frame(a = sum(.x, .y))),
    data.frame(a = rep(4, 3))
  )
  expect_equal(
    slide2_dfc(x, y, ~ data.frame(a = sum(.x, .y))),
...
test_that("pslide()", {
  expect_equal(
    pslide_dfr(list(x, y, z), ~ data.frame(a = ..1 + ..2 + ..3)),
    data.frame(a = 7:5)
  )
  expect_equal(
    pslide_dfc(list(x, y, z), ~ data.frame(a = ..1 + ..2 + ..3)),
